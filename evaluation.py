...
def compare_GED_graphs(gp_graphs_path, gt_graphs_path, take_first_result, max_time, out_path):
    ...
    for gp_graph_name in tqdm(os.listdir(gp_graphs_path)):
        ...
        if os.path.exists(f'{gt_graphs_path}/{gt_graph_name}'):
            ...
            for v in nx.optimize_graph_edit_distance(gt_graph, nx.MultiGraph()):
                GT_G0 = v
                break
            # print(min_result / (GP_G0 + GT_G0))
        else:
            pass
            # print(f'path: {gt_graphs_path}/{gt_graph_name} doesnt exist')
        # print(f'graded with min_result {min_result}')
    ...
    # print('mean GED:', sum(list(all_results.values())) / len(list(all_results.values())))
            ...
            if node != gt_n and node in matched_nodes.keys():
                ...
                if nx.has_path(Gp, gp_n, corr_node) and gp_n != corr_node:
                    ...
                    d_gp_len, d_gp_path = nx.single_source_dijkstra(Gp, gp_n,
                                                                    corr_node)  # nx.dijkstra_path_length(Gp, gp_n, corr_node) #nx.single_source_dijkstra(Gp, gp_n, corr_node) #nx.dijkstra_path_length(Gp, gp_n, corr_node)
...
def calc_F1_IoU_for_all(base_path, post_proc_state, gt_path):
    ...
    for img in tqdm(os.listdir(data_path)):
        ...
        # print(tp/total, fp/total, fn/total, tn/total, sum([tp/total, fp/total, fn/total, tn/total]))
    ...
    print(sum(tps) / len(tps), sum(fps) / len(fps), sum(fns) / len(fns), sum(tns) / len(tns),
          sum([sum(tps) / len(tps), sum(fps) / len(fps), sum(fns) / len(fns), sum(tns) / len(tns)]))
